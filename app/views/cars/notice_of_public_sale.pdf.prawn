prawn_document() do |pdf|
  ...
  pdf.move_down 20
  pdf.font_size 12
  pdf.text "<u>On: #{(@car.mail_notice_of_lien_date + 44.days).to_s(:short_date)}</u> at 5:00 pm the undersigned will offer to tell the following described vehicle to satisfy the charges for towing, storage, labor, and any other charges against the vehicle listed below.", inline_:format => true
  ...
  pdf.span(pdf.bounds.width - 10, position: 10) do
    pdf.text "<u>Year:</u>   #{@car.year}      <u>Make:</u>   #{@car.make}      <u>Model:</u>   #{@car.model}", inline_:format => true, leading: 30
    pdf.text "<u>VIN:</u>   #{@car.vin}   <u>Titled:</u>   #{@car.owner_name}   <u>LP #:</u>   #{@car.license_plate_number}", inline_:format => true
    
    #TODO Does titled refer to the registered owner?
  end
    
  pdf.move_down 15
  pdf.text "<u>Total Amount Owed:</u>   #{number_to_currency(@car.charge_total)}", inline_:format => true
  ...
  pdf.text "Date of Posting: <u>   #{(@car.mail_notice_of_lien_date + 10.days).to_s(:short_date)}</u>", inline_:format => true
end
